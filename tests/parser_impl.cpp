...
TEST(SCALE, BadTX) {
  ...
  uint8_t buffer[100];
  auto bufferLen =
      parseHexString(buffer, sizeof(buffer), "0102030405060708091011");
  ...
  err = _readTx(&ctx, &tx);
  EXPECT_EQ(err, parser_unexpected_buffer_end)
...
TEST(SCALE, TransferTXBadTxVersion) {
  ...
  const auto testTx = "0400ff8d16d62802ca55326ec52bf76a8543b90e2aba5bcf6cd195c0"
  ...
  err = _readTx(&ctx, &tx);
  EXPECT_EQ(err, parser_tx_version_not_supported)
      << parser_getErrorDescription(err);
...
TEST(SCALE, TransferTXBadSpec) {
  ...
  const auto testTx = "00004d3dcb99d5038d240b63ce64c10c05010000000300000091b171"
