...
parser_error_t _readCallImpl(parser_context_t *c, pd_Call_t *v,
  ...
  CHECK_ERROR(_readMethod(c, v->callIndex.moduleIdx, v->callIndex.idx,
                          (pd_Method_t *)m))
...
parser_error_t _readCompactAccountIndex(parser_context_t *c,
                                        pd_CompactAccountIndex_t *v) {
...
parser_error_t _readu128(parser_context_t *c,
                         pd_u128_t *v){GEN_DEF_READARRAY(16)}

parser_error_t
    _readAccountIdLookupOfT(parser_context_t *c, pd_AccountIdLookupOfT_t *v) {
  CHECK_INPUT()
...
parser_error_t _readCommissionClaimPermissionAccountId(
    parser_context_t *c, pd_CommissionClaimPermissionAccountId_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0:
...
parser_error_t _readTuplePerbillAccountId(parser_context_t *c,
                                          pd_TuplePerbillAccountId_t *v) {
  CHECK_INPUT()
...
parser_error_t _readWeightMaximumFee(parser_context_t *c,
                                     pd_WeightMaximumFee_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0: // None
...
parser_error_t _readBondExtraBalanceOfT(parser_context_t *c,
                                        pd_BondExtraBalanceOfT_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0:
...
parser_error_t
_readCommissionChangeRateBlockNumber(parser_context_t *c,
                                     pd_CommissionChangeRateBlockNumber_t *v) {
  CHECK_INPUT()
...
parser_error_t _readConfigOpAccountId(parser_context_t *c,
                                      pd_ConfigOpAccountId_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0: // Noop
...
parser_error_t _readConfigOpBalanceOfT(parser_context_t *c,
                                       pd_ConfigOpBalanceOfT_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0: // Noop
...
parser_error_t _readConfigOpPerbill(parser_context_t *c,
                                    pd_ConfigOpPerbill_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0: // Noop
...
parser_error_t _readConfigOpPercent(parser_context_t *c,
                                    pd_ConfigOpPercent_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  switch (v->value) {
  case 0: // Noop
...
parser_error_t _readDispatchFeeModifier(parser_context_t *c,
                                        pd_DispatchFeeModifier_t *v) {
  CHECK_INPUT()
...
parser_error_t _readAdjustmentDirection(parser_context_t *c,
                                        pd_AdjustmentDirection_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  if (v->value > 1) {
    return parser_value_out_of_range;
...
parser_error_t _readAppData(parser_context_t *c,
                            pd_AppData_t *v){GEN_DEF_READVECTOR(u8)}

parser_error_t
    _readAppKey(parser_context_t *c, pd_AppKey_t *v){GEN_DEF_READVECTOR(u8)}

parser_error_t
    _readClaimPermission(parser_context_t *c, pd_ClaimPermission_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->value))
  if (v->value > 3) {
    return parser_value_out_of_range;
...
parser_error_t
    _readKeys(parser_context_t *c, pd_Keys_t *v){GEN_DEF_READARRAY(4 * 32)}
...
parser_error_t _readOptionCommissionClaimPermissionAccountId(
    parser_context_t *c, pd_OptionCommissionClaimPermissionAccountId_t *v) {
  CHECK_INPUT()
...
parser_error_t
_readOptionTuplePerbillAccountId(parser_context_t *c,
                                 pd_OptionTuplePerbillAccountId_t *v) {
  CHECK_INPUT()
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _toStringCommissionClaimPermissionAccountId(
    const pd_CommissionClaimPermissionAccountId_t *v, char *outValue,
...
parser_error_t
_toStringTuplePerbillAccountId(const pd_TuplePerbillAccountId_t *v,
                               char *outValue, uint16_t outValueLen,
                               uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  uint8_t pages[2] = {0};
  CHECK_ERROR(
      _toStringPerbill(&v->perbill, outValue, outValueLen, 0, &pages[0]))
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[0]) {
    CHECK_ERROR(_toStringPerbill(&v->perbill, outValue, outValueLen, pageIdx,
                                 &pages[0]))
    return parser_ok;
  }
  pageIdx -= pages[0];
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(
        _toStringAccountId(&v->id, outValue, outValueLen, pageIdx, &pages[1]))
    return parser_ok;
  }
...
parser_error_t _toStringWeightFee(const pd_WeightFee_t *v, char *outValue,
                                  uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
  ...
  case 1: // some
    CHECK_ERROR(
        _toStringu32(&v->some, outValue, outValueLen, pageIdx, pageCount))
    break;
  default:
    return parser_unexpected_value;
  }
...
parser_error_t _toStringWeightMaximumFee(const pd_WeightMaximumFee_t *v,
                                         char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
  ...
  case 1: // some
    CHECK_ERROR(
        _toStringu128(&v->some, outValue, outValueLen, pageIdx, pageCount))
    break;
  default:
    return parser_unexpected_value;
  }
...
parser_error_t _toStringBondExtraBalanceOfT(const pd_BondExtraBalanceOfT_t *v,
                                            char *outValue,
                                            uint16_t outValueLen,
                                            uint8_t pageIdx,
                                            uint8_t *pageCount) {
  ...
  *pageCount = 1;
  switch (v->value) {
  case 0:
    CHECK_ERROR(_toStringBalance(&v->freeBalance, outValue, outValueLen,
                                 pageIdx, pageCount))
    break;
  case 1:
...
parser_error_t _toStringCommissionChangeRateBlockNumber(
    const pd_CommissionChangeRateBlockNumber_t *v, char *outValue,
    uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  uint8_t pages[2] = {0};
  CHECK_ERROR(
      _toStringPerbill(&v->maxIncrease, outValue, outValueLen, 0, &pages[0]))
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[0]) {
    CHECK_ERROR(_toStringPerbill(&v->maxIncrease, outValue, outValueLen,
                                 pageIdx, &pages[0]))
    return parser_ok;
  }
  pageIdx -= pages[0];
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(
        _toStringu32(&v->minDelay, outValue, outValueLen, pageIdx, &pages[1]))
    return parser_ok;
  }
...
parser_error_t _toStringConfigOpAccountId(const pd_ConfigOpAccountId_t *v,
                                          char *outValue, uint16_t outValueLen,
                                          uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
  ...
  case 1:
    CHECK_ERROR(
        _toStringAccountId(&v->set, outValue, outValueLen, pageIdx, pageCount))
    break;
  case 2:
...
parser_error_t _toStringConfigOpBalanceOfT(const pd_ConfigOpBalanceOfT_t *v,
                                           char *outValue, uint16_t outValueLen,
                                           uint8_t pageIdx,
                                           uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
  ...
  case 1:
    CHECK_ERROR(
        _toStringBalance(&v->set, outValue, outValueLen, pageIdx, pageCount))
    break;
  case 2:
    snprintf(outValue, outValueLen, "Remove");
    break;
...
parser_error_t _toStringConfigOpPerbill(const pd_ConfigOpPerbill_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
  ...
  case 1:
    CHECK_ERROR(
        _toStringPerbill(&v->set, outValue, outValueLen, pageIdx, pageCount))
    break;
  case 2:
    snprintf(outValue, outValueLen, "Remove");
    break;
...
parser_error_t _toStringConfigOpPercent(const pd_ConfigOpPercent_t *v,
  ...
  *pageCount = 1;
  ...
  case 1:
    CHECK_ERROR(
        _toStringPercent(&v->set, outValue, outValueLen, pageIdx, pageCount))
    break;
  case 2:
    snprintf(outValue, outValueLen, "Remove");
    break;
...
parser_error_t _toStringDispatchFeeModifier(const pd_DispatchFeeModifier_t *v,
                                            char *outValue,
                                            uint16_t outValueLen,
                                            uint8_t pageIdx,
                                            uint8_t *pageCount) {
  ...
  // First measure number of pages
  uint8_t pages[3] = {0};
  CHECK_ERROR(_toStringWeightMaximumFee(&v->weight_maximum_fee, outValue,
                                        outValueLen, 0, &pages[0]))
  CHECK_ERROR(_toStringWeightFee(&v->weight_fee_divider, outValue, outValueLen,
                                 0, &pages[1]))
  CHECK_ERROR(_toStringWeightFee(&v->weight_fee_multiplier, outValue,
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(_toStringWeightFee(&v->weight_fee_divider, outValue,
...
parser_error_t _toStringValidatorPrefs(const pd_ValidatorPrefs_t *v,
  ...
  // First measure number of pages
  uint8_t pages[2] = {0};
  CHECK_ERROR(_toStringCompactPerBill(&v->commission, outValue, outValueLen, 0,
                                      &pages[0]))
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[0]) {
    CHECK_ERROR(_toStringCompactPerBill(&v->commission, outValue, outValueLen,
                                        pageIdx, &pages[0]))
    return parser_ok;
  }
  pageIdx -= pages[0];
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(
        _toStringbool(&v->blocked, outValue, outValueLen, pageIdx, &pages[1]))
    return parser_ok;
  }
...
parser_error_t _toStringAdjustmentDirection(const pd_AdjustmentDirection_t *v,
                                            char *outValue,
                                            uint16_t outValueLen,
                                            uint8_t pageIdx,
                                            uint8_t *pageCount) {
...
parser_error_t _toStringAppData(const pd_AppData_t *v, char *outValue,
                                uint16_t outValueLen, uint8_t pageIdx,
                                uint8_t *pageCount){GEN_DEF_TOSTRING_VECTOR(u8)}

parser_error_t _toStringAppKey(const pd_AppKey_t *v, char *outValue,
                               uint16_t outValueLen, uint8_t pageIdx,
                               uint8_t *pageCount){GEN_DEF_TOSTRING_VECTOR(u8)}

parser_error_t _toStringClaimPermission(const pd_ClaimPermission_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  UNUSED(pageIdx);
...
parser_error_t _toStringConfigOpu32(const pd_ConfigOpu32_t *v, char *outValue,
                                    uint16_t outValueLen, uint8_t pageIdx,
                                    uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  *pageCount = 1;
...
parser_error_t
    _toStringPage(const pd_Page_t *v, char *outValue, uint16_t outValueLen,
                  uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  CHECK_ERROR(
      _toStringu32(&v->value, outValue, outValueLen, pageIdx, pageCount))
  return parser_ok;
}

parser_error_t _toStringPoolId(const pd_PoolId_t *v, char *outValue,
                               uint16_t outValueLen, uint8_t pageIdx,
                               uint8_t *pageCount) {
  return _toStringu32(&v->value, outValue, outValueLen, pageIdx, pageCount);
}

parser_error_t _toStringPoolState(const pd_PoolState_t *v, char *outValue,
                                  uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  UNUSED(pageIdx);
...
parser_error_t
_toStringVecAccountIdLookupOfT(const pd_VecAccountIdLookupOfT_t *v,
                               char *outValue, uint16_t outValueLen,
                               uint8_t pageIdx, uint8_t *pageCount) {
...
parser_error_t _toStringOptionCommissionClaimPermissionAccountId(
    const pd_OptionCommissionClaimPermissionAccountId_t *v, char *outValue,
    uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t
_toStringOptionTuplePerbillAccountId(const pd_OptionTuplePerbillAccountId_t *v,
                                     char *outValue, uint16_t outValueLen,
                                     uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
