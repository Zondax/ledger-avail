...
parser_error_t _readFungibleToken(parser_context_t *c, pd_FungibleToken_t *v) {
    CHECK_INPUT()
    CHECK_ERROR(_readH256(c, &v->asset_id))
...
parser_error_t _readu128(parser_context_t *c, pd_u128_t *v){GEN_DEF_READARRAY(16)}
...
parser_error_t _readMessage(parser_context_t *c, pd_Message_t *v) {
    ...
    switch (v->value) {
        case 0:  // BoundedData
            CHECK_ERROR(_readBytes(c, &v->bounded_data))
            break;
        case 1:  // FungibleToken
            CHECK_ERROR(_readFungibleToken(c, &v->fungible_token))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _readWeightFee(parser_context_t *c, pd_WeightFee_t *v) {
    ...
    switch (v->value) {
        case 0:  // None
            break;
        case 1:  // some
            CHECK_ERROR(_readUInt32(c, &v->some))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _readWeightMaximumFee(parser_context_t *c, pd_WeightMaximumFee_t *v) {
    ...
    switch (v->value) {
        case 0:  // None
            break;
        case 1:  // some
            CHECK_ERROR(_readu128(c, &v->some))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _readAddressedMessage(parser_context_t *c, pd_AddressedMessage_t *v) {
    CHECK_INPUT()
    CHECK_ERROR(_readMessage(c, &v->message))
    CHECK_ERROR(_readH256(c, &v->from))
    CHECK_ERROR(_readH256(c, &v->to))
    CHECK_ERROR(_readCompactu32(c, &v->origin_domain))
    CHECK_ERROR(_readCompactu32(c, &v->destination_domain))
    CHECK_ERROR(_readCompactu64(c, &v->id))
...
parser_error_t _readDispatchFeeModifier(parser_context_t *c, pd_DispatchFeeModifier_t *v) {
    ...
    CHECK_ERROR(_readWeightMaximumFee(c, &v->weight_maximum_fee))
    CHECK_ERROR(_readWeightFee(c, &v->weight_fee_divider))
    CHECK_ERROR(_readWeightFee(c, &v->weight_fee_multiplier))
...
parser_error_t _readValidProof(parser_context_t *c, pd_ValidProof_t *v) {
    CHECK_ERROR(_readVecBytes(c, &v->valid_proof))
...
parser_error_t _readAppData(parser_context_t *c, pd_AppData_t *v){GEN_DEF_READVECTOR(u8)}

parser_error_t _readAppKey(parser_context_t *c, pd_AppKey_t *v){GEN_DEF_READVECTOR(u8)}
...
parser_error_t _readKeys(parser_context_t *c, pd_Keys_t *v){GEN_DEF_READARRAY(4 * 32)}
...
parser_error_t _readVecBytes(parser_context_t *c, pd_VecBytes_t *v){GEN_DEF_READVECTOR(Bytes)}
...
parser_error_t _readOptionCommissionClaimPermissionAccountId(parser_context_t *c,
                                                             pd_OptionCommissionClaimPermissionAccountId_t *v) {
    CHECK_INPUT()
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
...
parser_error_t _toStringData(const pd_Data_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    *pageCount = 1;
    if (v->value == 0) {
...
parser_error_t _toStringH256(const pd_H256_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
...
parser_error_t _toStringFungibleToken(const pd_FungibleToken_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                      uint8_t *pageCount) {
    CLEAN_AND_CHECK()

    // First measure number of pages
    uint8_t pages[2] = {0};
    CHECK_ERROR(_toStringH256(&v->asset_id, outValue, outValueLen, 0, &pages[0]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringH256(&v->asset_id, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];
...
parser_error_t _toStringTupleDataData(const pd_TupleDataData_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                      uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringu128(const pd_u128_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringCommissionClaimPermissionAccountId(const pd_CommissionClaimPermissionAccountId_t *v, char *outValue,
...
parser_error_t _toStringMessage(const pd_Message_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    *pageCount = 1;
    switch (v->value) {
        case 0:  // BoundedData
            CHECK_ERROR(_toStringBytes(&v->bounded_data, outValue, outValueLen, pageIdx, pageCount))
            break;
        case 1:  // FungibleToken
            CHECK_ERROR(_toStringFungibleToken(&v->fungible_token, outValue, outValueLen, pageIdx, pageCount))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _toStringTuplePerbillAccountId(const pd_TuplePerbillAccountId_t *v, char *outValue, uint16_t outValueLen,
                                              uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringWeightFee(const pd_WeightFee_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    *pageCount = 1;
    switch (v->value) {
        case 0:  // None
            ...
            break;
        case 1:  // some
            CHECK_ERROR(_toStringu32(&v->some, outValue, outValueLen, pageIdx, pageCount))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _toStringWeightMaximumFee(const pd_WeightMaximumFee_t *v, char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    *pageCount = 1;
    switch (v->value) {
        case 0:  // None
            ...
            break;
        case 1:  // some
            CHECK_ERROR(_toStringu128(&v->some, outValue, outValueLen, pageIdx, pageCount))
            break;
        default:
            return parser_unexpected_value;
    }
...
parser_error_t _toStringschedulePeriodBlockNumber(const pd_schedulePeriodBlockNumber_t *v, char *outValue,
                                                  uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringAddressedMessage(const pd_AddressedMessage_t *v, char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()

    // First measure number of pages
    uint8_t pages[6] = {0};
    CHECK_ERROR(_toStringMessage(&v->message, outValue, outValueLen, 0, &pages[0]))
    CHECK_ERROR(_toStringH256(&v->from, outValue, outValueLen, 0, &pages[1]))
    CHECK_ERROR(_toStringH256(&v->to, outValue, outValueLen, 0, &pages[2]))
    CHECK_ERROR(_toStringCompactu32(&v->origin_domain, outValue, outValueLen, 0, &pages[3]))
    CHECK_ERROR(_toStringCompactu32(&v->destination_domain, outValue, outValueLen, 0, &pages[4]))
    CHECK_ERROR(_toStringCompactu64(&v->id, outValue, outValueLen, 0, &pages[5]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringMessage(&v->message, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];

    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringH256(&v->from, outValue, outValueLen, pageIdx, &pages[1]))
        return parser_ok;
    }
    pageIdx -= pages[1];

    if (pageIdx < pages[2]) {
        CHECK_ERROR(_toStringH256(&v->to, outValue, outValueLen, pageIdx, &pages[2]))
        return parser_ok;
    }
    pageIdx -= pages[2];

    if (pageIdx < pages[3]) {
        CHECK_ERROR(_toStringCompactu32(&v->origin_domain, outValue, outValueLen, pageIdx, &pages[3]))
        return parser_ok;
    }
    pageIdx -= pages[3];

    if (pageIdx < pages[4]) {
        CHECK_ERROR(_toStringCompactu32(&v->destination_domain, outValue, outValueLen, pageIdx, &pages[4]))
        return parser_ok;
    }
    pageIdx -= pages[4];

    if (pageIdx < pages[5]) {
        CHECK_ERROR(_toStringCompactu64(&v->id, outValue, outValueLen, pageIdx, &pages[5]))
        return parser_ok;
    }
...
parser_error_t _toStringCommissionChangeRateBlockNumber(const pd_CommissionChangeRateBlockNumber_t *v, char *outValue,
                                                        uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringDispatchFeeModifier(const pd_DispatchFeeModifier_t *v, char *outValue, uint16_t outValueLen,
                                            uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()

    // First measure number of pages
    uint8_t pages[3] = {0};
    CHECK_ERROR(_toStringWeightMaximumFee(&v->weight_maximum_fee, outValue, outValueLen, 0, &pages[0]))
    CHECK_ERROR(_toStringWeightFee(&v->weight_fee_divider, outValue, outValueLen, 0, &pages[1]))
    CHECK_ERROR(_toStringWeightFee(&v->weight_fee_multiplier, outValue, outValueLen, 0, &pages[2]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringWeightMaximumFee(&v->weight_maximum_fee, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];

    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringWeightFee(&v->weight_fee_divider, outValue, outValueLen, pageIdx, &pages[1]))
        return parser_ok;
    }
    pageIdx -= pages[1];

    if (pageIdx < pages[2]) {
        CHECK_ERROR(_toStringWeightFee(&v->weight_fee_multiplier, outValue, outValueLen, pageIdx, &pages[2]))
        return parser_ok;
    }
...
parser_error_t _toStringValidProof(const pd_ValidProof_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                   uint8_t *pageCount) {
    CLEAN_AND_CHECK()

    // First measure number of pages
    uint8_t pages[1] = {0};
    CHECK_ERROR(_toStringVecBytes(&v->valid_proof, outValue, outValueLen, 0, &pages[0]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringVecBytes(&v->valid_proof, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
...
parser_error_t _toStringAdjustmentDirection(const pd_AdjustmentDirection_t *v, char *outValue, uint16_t outValueLen,
                                            uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    UNUSED(pageIdx);
    *pageCount = 1;
...
parser_error_t _toStringAppData(const pd_AppData_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                uint8_t *pageCount){GEN_DEF_TOSTRING_VECTOR(u8)}

parser_error_t _toStringAppKey(const pd_AppKey_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                               uint8_t *pageCount){GEN_DEF_TOSTRING_VECTOR(u8)}

parser_error_t _toStringClaimPermission(const pd_ClaimPermission_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                        uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    UNUSED(pageIdx);
    switch (v->value) {
...
parser_error_t _toStringOptionschedulePeriodBlockNumber(const pd_OptionschedulePeriodBlockNumber_t *v, char *outValue,
                                                        uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()
    *pageCount = 1;
    switch (v->value) {
...
parser_error_t _toStringschedulePriority(const pd_schedulePriority_t *v, char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
    return _toStringu8(&v->value, outValue, outValueLen, pageIdx, pageCount);
}

parser_error_t _toStringVecBytes(const pd_VecBytes_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                 uint8_t *pageCount) {
...
parser_error_t _toStringOptionCommissionClaimPermissionAccountId(const pd_OptionCommissionClaimPermissionAccountId_t *v,
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringCommissionClaimPermissionAccountId(&v->contained, outValue, outValueLen, pageIdx, pageCount));
    } else {
        snprintf(outValue, outValueLen, "None");
    }
    return parser_ok;
...
parser_error_t _toStringOptionTimepoint(const pd_OptionTimepoint_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                        uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringOptionTuplePerbillAccountId(const pd_OptionTuplePerbillAccountId_t *v, char *outValue,
                                                    uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
    CLEAN_AND_CHECK()

    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringTuplePerbillAccountId(&v->contained, outValue, outValueLen, pageIdx, pageCount));
    } else {
        snprintf(outValue, outValueLen, "None");
    }
    return parser_ok;
...
parser_error_t _toStringOptionAccountId(const pd_OptionAccountId_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                        uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringOptionProxyType(const pd_OptionProxyType_t *v, char *outValue, uint16_t outValueLen, uint8_t pageIdx,
                                        uint8_t *pageCount) {
    CLEAN_AND_CHECK()
