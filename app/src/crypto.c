...
static zxerr_t crypto_extractPublicKey(key_kind_e addressKind, uint8_t *pubKey,
                                       uint16_t pubKeyLen) {
  ...
  CATCH_CXERROR(os_derive_bip32_with_seed_no_throw(
      HDW_NORMAL, CX_CURVE_Ed25519, hdPath, HDPATH_LEN_DEFAULT, privateKeyData,
      NULL, NULL, 0));

  switch (addressKind) {
  case key_ed25519: {
    CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(
        CX_CURVE_Ed25519, privateKeyData, 32, &cx_privateKey));
    CATCH_CXERROR(cx_ecfp_init_public_key_no_throw(CX_CURVE_Ed25519, NULL, 0,
                                                   &cx_publicKey));
    CATCH_CXERROR(cx_ecfp_generate_pair_no_throw(
        CX_CURVE_Ed25519, &cx_publicKey, &cx_privateKey, 1));
...
zxerr_t crypto_sign_ed25519(uint8_t *signature, uint16_t signatureMaxlen,
  ...
  if (signature == NULL || message == NULL ||
      signatureMaxlen < SIG_PLUS_TYPE_LEN || messageLen == 0) {
    return zxerr_buffer_too_small;
  }
  cx_ecfp_private_key_t cx_privateKey;
  uint8_t privateKeyData[SK_LEN_25519] = {0};
  ...
  if (messageLen > MAX_SIGN_SIZE) {
    ...
    CATCH_CXERROR(cx_hash_no_throw(&ctx.header, CX_LAST, message, messageLen,
  ...
  CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(
      CX_CURVE_Ed25519, privateKeyData, SCALAR_LEN_ED25519, &cx_privateKey));

  // Sign
  *signature = PREFIX_SIGNATURE_TYPE_ED25519;
  CATCH_CXERROR(cx_eddsa_sign_no_throw(&cx_privateKey, CX_SHA512, toSign,
                                       messageLen, signature + 1,
                                       signatureMaxlen - 1));
  error = zxerr_ok;
...
static zxerr_t crypto_sign_sr25519_helper(const uint8_t *data, size_t len) {
  ...
  *sr25519_signature = PREFIX_SIGNATURE_TYPE_SR25519;
  sign_sr25519_phase1(privateKeyData, pubkey, NULL, 0, data, len,
                      sr25519_signature + 1);
  CATCH_CXERROR(crypto_scalarmult_ristretto255_base_sdk(
      sr25519_signature + 1, sr25519_signature + 1 + PK_LEN_25519));
  error = zxerr_ok;

catch_cx_error:
  if (error == zxerr_ok) {
    sign_sr25519_phase2((const uint8_t *)privateKeyData,
                        (const uint8_t *)pubkey, NULL, 0, data, len,
                        sr25519_signature + 1);
  } else {
    explicit_bzero(sr25519_signature, sizeof(sr25519_signature));
  }
...
zxerr_t crypto_sign_sr25519(const uint8_t *message, size_t messageLen) {
  ...
  if (messageLen > MAX_SIGN_SIZE) {
    ...
    if ((cx_blake2b_init_no_throw(&ctx, 256) != CX_OK) ||
        (cx_hash_no_throw(&ctx.header, CX_LAST, message, messageLen,
                          messageDigest, BLAKE2B_DIGEST_SIZE) != CX_OK)) {
...
zxerr_t crypto_fillAddress(key_kind_e addressKind, uint8_t *buffer,
  ...
  size_t outLen = crypto_SS58EncodePubkey(
      buffer + PK_LEN_25519, bufferLen - PK_LEN_25519, PK_ADDRESS_TYPE, buffer);
  if (outLen == 0) {
    MEMZERO(buffer, bufferLen);
    return zxerr_unknown;
  }
