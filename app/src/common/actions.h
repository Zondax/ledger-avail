...
__Z_INLINE void app_return_sr25519() {
  const zxerr_t err =
      copy_sr25519_signdata(G_io_apdu_buffer, sizeof(G_io_apdu_buffer) - 2);
...
__Z_INLINE zxerr_t app_fill_address(key_kind_e addressKind) {
  ...
  return crypto_fillAddress(addressKind, G_io_apdu_buffer,
                            IO_APDU_BUFFER_SIZE - 2, &action_addrResponseLen);
}
