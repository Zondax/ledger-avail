...
const char *tx_raw_parse() {
  ...
  if (strncmp((const char *)data, prefix, prefixLen) != 0 ||
      strncmp((const char *)data + dataLen - postfixLen, postfix, postfixLen) !=
          0) {
    return parser_getErrorDescription(parser_unexpected_value);
  }
...
zxerr_t tx_raw_getItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen,
                       char *outVal, uint16_t outValLen, uint8_t pageIdx,
                       uint8_t *pageCount) {
  ...
  if (allPrintable) {
    ...
    pageStringExt(outVal, outValLen, (const char *)buf, bufLen, pageIdx,
                  pageCount);
  } else {
    snprintf(outKey, outKeyLen, "Payload (hex)");
    pageStringHex(outVal, outValLen, (const char *)buf, bufLen, pageIdx,
                  pageCount);
  }
