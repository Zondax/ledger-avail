...
cx_err_t ss58hash(const unsigned char *in, unsigned int inLen,
  ...
  CHECK_CXERROR(cx_hash_no_throw(&ctx.header, 0, SS58_BLAKE_PREFIX,
                                 SS58_BLAKE_PREFIX_LEN, NULL, 0));
...
int ss58hash(const unsigned char *in, unsigned int inLen, unsigned char *out,
             unsigned int outLen) {
  blake2b_state s;
  blake2b_init(&s, 64);
